			...
			<main className='max-w-6xl mx-auto p-4 sm:p-8'>
				...
				<div className='bg-white dark:bg-slate-900 rounded-3xl shadow-xl border p-6 lg:p-10 flex flex-col lg:flex-row gap-8 mb-10'>
					<div className='flex-1'>
						<img
							src={post.imageUrl}
							className='rounded-2xl w-full aspect-video object-cover border'
							alt='PC'
						/>
						<h2 className='mt-4 text-3xl font-black text-slate-900 dark:text-white uppercase italic'>
							{post.caption}
						</h2>
					</div>
					<div className='flex-1 bg-slate-50 dark:bg-slate-800 p-8 rounded-2xl border'>
						<h2 className='text-2xl font-bold text-blue-600 mb-6 underline'>
							Technical Specs:
						</h2>
						<ul className='space-y-4'>
							<li className='flex items-center gap-3'>
								<Cpu className='text-blue-500' /> <strong>CPU:</strong>{" "}
								{post.cpu}
							</li>
							<li className='flex items-center gap-3'>
								<MonitorPlay className='text-violet-500' />{" "}
				...
				<div className='mt-10 bg-blue-600 p-8 rounded-3xl text-white shadow-xl relative overflow-hidden'>
					{!user && (
						<div className='absolute inset-0 bg-black/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center text-center p-6'>
							...
							<Link
								href='/signin'
								className='bg-white text-blue-600 px-8 py-2 rounded-full font-bold hover:bg-slate-100 transition-all'
							>
								Sign In
							</Link>
						</div>
					...
					<form onSubmit={handleSubmitReview} className='space-y-4'>
						<div className='flex flex-col gap-2'>
							<label className='text-sm font-bold opacity-80 uppercase'>
								Rating (1-5)
							</label>
							<input
								type='number'
								...
								className='w-24 p-4 rounded-xl text-slate-900 font-bold outline-none'
							/>
						</div>
						<div className='flex flex-col gap-2'>
							<label className='text-sm font-bold opacity-80 uppercase'>
							...
							<textarea
								...
								className='w-full p-5 rounded-2xl text-slate-900 h-32 outline-none'
						...
						<button
							type='submit'
							className='bg-white text-blue-600 font-black py-4 px-10 rounded-2xl flex items-center gap-2'
						>
							<Send size={20} /> SUBMIT AS{" "}
