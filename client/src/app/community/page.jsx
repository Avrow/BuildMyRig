import { Calendar, Cpu, MonitorPlay, MemoryStick, Users, Star, MessageSquare } from "lucide-react";
...
async function getAllBuilds() {
    await connectDB();
    // author name and id populate
    const raw = await BuildPost.find({}).populate('author', 'name _id').sort({ createdAt: -1 }).lean();

    return Promise.all(raw.map(async (b) => {
        const reviewsRaw = await Review.find({ buildId: b._id }).lean();
        
        // original long mapping logic where we convert ObjectIds to strings and dates to ISO strings for both reviews and their nested replies
        const fixedReviews = reviewsRaw.map(rev => ({
            ...rev,
            _id: rev._id.toString(),
            buildId: rev.buildId.toString(),
            userId: rev.userId.toString(),
            createdAt: rev.createdAt.toISOString(),
            replies: (rev.replies || []).map(r => ({
                ...r, 
                _id: r._id?.toString(), 
        ...
        const avgRating = fixedReviews.length > 0 
            ? (fixedReviews.reduce((acc, r) => acc + r.rating, 0) / fixedReviews.length).toFixed(1) 
            : "0.0";

        return {
            id: b._id.toString(),
            authorId: b.author?._id?.toString(), // permission checks
            ...
            ram: b.ram,
            avgRating,
            commentCount: fixedReviews.length,
                            ...
                            <div className='p-6 flex flex-col flex-1'>
                                <div className='flex justify-between items-start mb-2'>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {currentUserId && String(currentUserId) === String(build.authorId) && (
                                            <PostActions post={build} currentUserId={String(currentUserId)} />
                                        )}
                                ...
                                <div className='grid grid-cols-3 gap-2 my-5'>
                                    <div className='bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl text-center flex flex-col items-center border border-slate-100 dark:border-slate-800 transition-colors group-hover:bg-blue-50/50 dark:group-hover:bg-blue-900/10'>
                                        ...
                                        <span className='text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate w-full'>{build.cpu}</span>
                                    </div>
                                    <div className='bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl text-center flex flex-col items-center border border-slate-200 dark:border-slate-800 transition-colors group-hover:bg-purple-50/50 dark:group-hover:bg-purple-900/10'>
                                        ...
                                        <span className='text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate w-full'>{build.gpu}</span>
                                    </div>
                                    <div className='bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl text-center flex flex-col items-center border border-slate-100 dark:border-slate-800 transition-colors group-hover:bg-green-50/50 dark:group-hover:bg-green-900/10'>
                                        ...
                                        <span className='text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate w-full'>{build.ram}</span>
                                    </div>
                                </div>
                                ...
                                <div className='mt-auto flex justify-between items-center text-[10px] text-slate-400 font-semibold uppercase tracking-wider'>
                                    <span className='flex items-center gap-1.5'>
                                        <MessageSquare size={13} className="text-blue-500"/> {build.commentCount} Feedback
                                    </span>
